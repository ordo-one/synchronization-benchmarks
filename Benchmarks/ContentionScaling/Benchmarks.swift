...
let csInstrSweep: [MutexVariant] = {
    ...
    if BenchEnv.instrument {
        v.append(.optimal(name: "INSTR Optimal", instrument: true))
        v.append(.parkingLot(name: "INSTR parking_lot", instrument: true))
...
let benchmarks: @Sendable () -> Void = {
    ...
    for cfg in configs {
        ...
        if BenchEnv.instrument && cfg.tasks >= 32 {
